...
case_files = [
    ...
    '/flare/hpe_dragon_collab/balin/PASC25/runs/ddict_cpu_bind/tiny_8192/2c_2s/mldocking_seq_ddict.o5504929',
...
keys = ['nodes','procs','load_IO_avg','load_IO_max','load_ddict_avg','load_ddict_max','inf_ddict_avg','inf_ddict_max','sort']
    ...
    with open(case,'r') as fh:
        ...
        for l in fh:
            ...
            if "Performed sorting of 10000 compounds" in l:
                cases[case]['sort'] = float(l.split(":")[-1].split(",")[-1].split("=")[-1])
...
for i, case in enumerate(cases):
    axs.bar(x+factors[i]*width, [cases[case]['load_IO_avg'], cases[case]['inf_ddict_avg'], cases[case]['sort']], width,label=legend[i])
axs.set_ylabel('Time [sec]')
axs.set_title('DDict Time')
