...
def run_docking(smiles, proc: int, num_procs: int):
    ...
    my_candidates = smiles
    num_candidates = len(my_candidates)        
    if debug: 
        with open(log_file_name,"a") as f:
            f.write(f"Docking worker assigned {len(my_candidates)} candidates\n")

    # if there are new candidates to simulate, run sims
    if len(my_candidates) > 0:
        tic = perf_counter()
        if not os.getenv("DOCKING_SIM_DUMMY"):
            scores = dock(my_candidates, proc, debug=debug) 
        else:
            sim_metrics = dummy_dock(my_candidates, proc, debug=debug) 

        toc = perf_counter()
        if debug:
            with open(log_file_name,"a") as f:
                f.write(f"proc {proc}: docking_sim_time {toc-tic} s\n")
    else:
        if debug:
            with open(log_file_name,"a") as f:
                f.write(f"no sims run")
...
def dock(candidates: List[str], proc: int, debug=False):
