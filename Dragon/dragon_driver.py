...
if __name__ == "__main__":
    ...
    node_counts = {"sorting": args.sorting_node_num, 
                    "training": 1, 
                    "inference": args.inference_node_num}
    node_counts["docking"] = num_tot_nodes - node_counts["sorting"] - node_counts["training"] - node_counts["inference"]

    if node_counts["docking"] <= 0:
        raise(f"Node partitioning not valid! {num_tot_nodes=} {num_docking_nodes=} {num_inference_nodes=} {num_sorting_nodes=}")
    ...
    if not args.only_load_data:
        ...
        tic = perf_counter()
        inf_proc = mp.Process(target=launch_inference, args=(data_dd, nodelists["inference"], num_procs, continue_event))
        inf_proc.start()
        inf_proc.join()
        toc = perf_counter()
        infer_time = toc - tic
